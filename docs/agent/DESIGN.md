...
- **Update (Roadmap Phase 1) ✅ (July 2026)**: the resolver used to collapse two different situations into `method="projected"` — a genuine future forecast, and a year that falls inside an already-signed, currently-active multi-year deal (it noted "under contract through YYYY" but the agent still "predicted" a number that was, in fact, already known). Fixed: `PhaseResolution` now carries `known_value` (`{contract_id, aav_millions, duration_years, total_value_millions}`), set for both `method="observed"` (an exact contract row) and the mid-contract case, renamed `method="known"` to distinguish it from a genuine `"projected"` forecast. `agent/predict/predict.py:run_prediction()` checks `known_value` first and, when set, builds the `ContractPrediction` directly from it (`_known_prediction()`, cited `source_type="tool"`) — **no LLM call at all**. Pushed as early as possible per Design Decision #7's architecture: `agent/intake/resolver.py:_attach_contract_status()` calls `resolve_phase()` right after intake resolves `player_id`/`target_year` (still harness-side, never the LLM's judgment) and attaches `contract_status`/`known_contract` to `IntakeResult`, so the orchestrator's system prompt skips calling `predict_tool` entirely — not just skipping the LLM inside it — whenever `contract_status="known"`. The backtest harness opts out via `run_prediction(..., force_predict=True)`, since it deliberately measures LLM accuracy against known historical outcomes and must not take the shortcut.
